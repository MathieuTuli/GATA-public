...
def expand_trajectories(
        trajectories, episode_no, batch_size, obs, actions,
        scores, dones, infos):
...
def evaluate_with_ground_truth_graph(env, agent, num_games):
    ...
    while(True):
        ...
        game_name_list += [game.metadata["uuid"].split("-")[-1]
        ...
        observation_strings, current_triplets, action_candidate_list, _, _ = agent.get_game_info_at_certain_step(
            obs, infos, prev_actions=None, prev_facts=None)
        ...
        for step_no in range(agent.eval_max_nb_steps_per_episode):
            ...
            trajectories = expand_trajectories(
                trajectories, game_id, batch_size, obs,
    ...
    print_strings.append("EVAL: rewards: {:2.3f} | normalized reward: {:2.3f} | used steps: {:2.3f}".format(
        np.mean(achieved_game_points), np.mean(normalized_game_points), np.mean(total_game_steps)))
    for i in range(len(game_name_list)):
        print_strings.append("game name: {}, reward: {:2.3f}, normalized reward: {:2.3f}, steps: {:2.3f}".format(
            game_name_list[i], achieved_game_points[i], normalized_game_points[i], total_game_steps[i]))
    print_strings.append(
        "======================================================")
    print_strings = "\n".join(print_strings)
    print(print_strings)
    with open('traj_test.pkl', 'wb') as f:
    ...
    return np.mean(achieved_game_points), np.mean(normalized_game_points), np.mean(total_game_steps), 0.0, print_strings, success
...
def evaluate(env, agent, num_games):
    ...
    while(True):
        ...
        observation_strings, current_triplets, action_candidate_list, _, current_game_facts = agent.get_game_info_at_certain_step(
            obs, infos, prev_actions=chosen_actions, prev_facts=None)
        observation_strings = [item + " <sep> " + a for item,
                               a in zip(observation_strings, chosen_actions)]
        still_running_mask = []
        final_scores = []
        ...
        for step_no in range(agent.eval_max_nb_steps_per_episode):
            ...
            observation_strings, current_triplets, action_candidate_list, _, current_game_facts = agent.get_game_info_at_certain_step(
                obs, infos, prev_actions=chosen_actions, prev_facts=prev_game_facts)
            observation_strings = [
                item + " <sep> " + a for item, a in zip(observation_strings, chosen_actions)]
...
def evaluate_belief_mode(env, agent, num_games):
    ...
    while(True):
        ...
        prev_h, prev_c = None, None
        observation_strings, _, action_candidate_list, target_command_strings, current_game_facts = agent.get_game_info_at_certain_step(
            obs, infos, prev_actions=chosen_actions, prev_facts=None, return_gt_commands=True)
        observation_strings = [item + " <sep> " + a for item,
                               a in zip(observation_strings, chosen_actions)]
        still_running_mask = []
        final_scores = []
        ...
        for step_no in range(agent.eval_max_nb_steps_per_episode):
            ...
            generated_commands = agent.command_generation_greedy_generation(
                observation_strings, triplets)
            triplets = agent.update_knowledge_graph_triplets(
                triplets, generated_commands)
            # choose what to do next from candidate list
            chosen_actions, chosen_indices, prev_h, prev_c = agent.act_greedy(
                observation_strings, triplets, action_candidate_list, prev_h, prev_c)
            # send chosen actions to game engine
            chosen_actions_before_parsing = [item[idx] for item, idx in zip(
                infos["admissible_commands"], chosen_indices)]
            obs, scores, dones, infos = env.step(chosen_actions_before_parsing)
            ...
            for i in range(batch_size):
                if still_running[i] == 0:
                    continue
                _, _, exact_f1 = get_match_result(
                    generated_commands[i], target_command_strings[i], type='exact')
        ...
        still_running_mask = np.array(still_running_mask)
        total_game_steps += np.sum(still_running_mask, 0).tolist()
        total_command_generation_f1 += [np.mean(item)
                                        for item in avg_command_generation_f1_in_a_game]
        game_id += batch_size
    ...
    print_strings.append(
        "======================================================")
    print_strings.append("EVAL: rewards: {:2.3f} | normalized reward: {:2.3f} | used steps: {:2.3f} | command generation f1: {:2.3f}".format(
        np.mean(achieved_game_points), np.mean(normalized_game_points), np.mean(total_game_steps), np.mean(total_command_generation_f1)))
    for i in range(len(game_name_list)):
        print_strings.append("game name: {}, reward: {:2.3f}, normalized reward: {:2.3f}, steps: {:2.3f}, cmd gen f1: {:2.3f}".format(
            game_name_list[i], achieved_game_points[i], normalized_game_points[i], total_game_steps[i], total_command_generation_f1[i]))
    print_strings.append(
        "======================================================")
    print_strings = "\n".join(print_strings)
    print(print_strings)
    ...
    success = np.mean([1 if x else 0 for x in infos['won']])
    return np.mean(achieved_game_points), np.mean(normalized_game_points), np.mean(total_game_steps), np.mean(total_command_generation_f1), print_strings, success
...
def evaluate_rl_with_real_graphs(env, agent, num_games):
    ...
    while(True):
        ...
        prev_step_dones = []
        prev_graph_hidden_state = torch.zeros(
            batch_size, agent.online_net.block_hidden_dim)
        if agent.use_cuda:
        ...
        ########
        # remove for obs_gen
        observation_strings, _, action_candidate_list, _, current_game_facts = agent.get_game_info_at_certain_step(
            obs, infos, prev_actions=chosen_actions, prev_facts=None, return_gt_commands=True)
        ########
        still_running_mask = []
        final_scores = []
...
def evaluate_pretrained_command_generation(env, agent, valid_test="valid", verbose=False):
    ...
    while(True):
        ...
        for i in range(len(observation_strings)):
            _, _, exact_f1 = get_match_result(
                pred_strings[i], target_strings[i], type='exact')
            _, _, soft_f1 = get_match_result(
                pred_strings[i], target_strings[i], type='soft')
            total_exact_f1.append(exact_f1)
            total_soft_f1.append(soft_f1)
            if verbose:
                to_print.append(str(counter) + " -------------------------------------------- exact f1: " +
                                str(exact_f1) + ", soft f1: " + str(soft_f1))
    ...
    with open(agent.experiment_tag + "_output.txt", "w") as f:
        f.write("\n".join(to_print))
    print("Hard F1: ", np.mean(np.array(total_exact_f1)),
          "Soft F1:", np.mean(np.array(total_soft_f1)))
    # success = np.mean([1 if x else 0 for x in infos['won']])
    # , success
...
def evaluate_observation_generation_free_generation(env, agent, valid_test="valid", verbose=False):
    ...
    while(True):
        ...
        max_len = max(lens)
        padded_observation_strings = [
            elem + ["<pad>"]*(max_len - len(elem)) for elem in observation_strings]
        padded_prev_action_strings = [
            elem + ["<pad>"]*(max_len - len(elem)) for elem in prev_action_strings]
        eps_masks = torch.zeros((batch_size, max_len), dtype=torch.float).cuda(
        ) if agent.use_cuda else torch.zeros((batch_size, max_len), dtype=torch.float)
        for i in range(batch_size):
            eps_masks[i, :lens[i]] = 1
        prev_h = None
        for j in range(max_len):
            batch_obs_string = [elem[j] for elem in padded_observation_strings]
            batch_prev_action_string = [elem[j]
                                        for elem in padded_prev_action_strings]
            pred_strings, prev_h = agent.observation_generation_greedy_generation(
                batch_obs_string, batch_prev_action_string, eps_masks, prev_h)

            for i in range(len(observation_strings)):
                if eps_masks[i, j] == 0:
                    continue  # if masked eps then don't compute F1
                _, _, f1 = get_match_result_obs_gen(
                    pred_strings[i], batch_obs_string[i])
                total_f1.append(f1)
                if verbose:
                    to_print.append(str(
                        counter) + " -------------------------------------------- soft f1: " + str(f1))
    ...
    while(True):
        ...
        for j in range(max_len):
            ...
            batch_prev_action_string = [elem[j]
                                        for elem in padded_prev_action_strings]
            with torch.no_grad():
                loss, _, prev_h = agent.observation_generation_teacher_force(
                    batch_obs_string, batch_prev_action_string, eps_masks[:, j], prev_h)
...
def evaluate_observation_infomax(env, agent, valid_test="valid"):
    ...
    while(True):
        observation_strings, prev_action_strings = env.get_batch()
        with torch.no_grad():
            valid_loss, acc = agent.get_observation_infomax_loss(
                observation_strings, prev_action_strings, evaluate=True)
...
def evaluate_action_prediction(env, agent, valid_test="valid", verbose=False):
    ...
    while(True):
        ...
        with torch.no_grad():
            loss, ap_ret, np_labels, action_choices = agent.get_action_prediction_logits(
        ...
        if verbose:
            for i in range(len(current_graph)):
                ...
                to_print.append("PRED ACTION: %s " %
                                (action_choices[i][pred[i]]))
    ...
    while(True):
        ...
        with torch.no_grad():
            loss, labels, dgi_discriminator_logits, batch_nonzero_idx = agent.get_deep_graph_infomax_logits(
                triplets)
        # sigmoid
        dgi_discriminator_logits = 1.0 / \
            (1.0 + np.exp(-dgi_discriminator_logits))
        ...
        for i in range(len(triplets)):
            ...
            ) + (batch_nonzero_idx[i] + len(agent.node_vocab)).tolist())
