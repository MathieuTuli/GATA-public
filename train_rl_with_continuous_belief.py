...
def train():
    ...
    # training game env
    agent.experiment_tag += f"seed{config['general']['random_seed']}_level{config['rl']['difficulty_level']}"
    ...
    running_avg_success = HistoryScoreCache(capacity=500)
    ...
    json_file_name = agent.experiment_tag.replace(
        ...
        f"_{config['general']['random_seed']}"
    ...
    if os.path.exists(data_dir + "/" + agent.load_graph_generation_model_from_tag + ".pt"):
        ...
        print("No real-valued graph generation module detected... Please check ",
              data_dir + "/" + agent.load_graph_generation_model_from_tag + ".pt")
    ...
    perfect_training = 0
    import tqdm
    for episode_no in tqdm.tqdm(range(0, agent.max_episode,
                                agent.batch_size)):
        ...
        # inverse of `prev_step_dones`
        episodes_masks = 1 - torch.tensor(prev_step_dones)
        ...
        for step_no in range(agent.max_nb_steps_per_episode):
            ...
            new_chosen_actions, chosen_indices, prev_h, prev_c = agent.act(
                observation_strings, new_adjacency_matrix, action_candidate_list, previous_h=prev_h, previous_c=prev_c, random=act_randomly)
            replay_info = [observation_strings, action_candidate_list, chosen_indices, generic.to_np(
                prev_graph_hidden_state), chosen_actions]
            ...
            prev_rewards = scores
            step_graph_rewards = [0.0 for _ in range(
                batch_size)]  # adding for obs_gen
            # counting bonus
            if agent.count_reward_lambda > 0:
        ...
        for b in range(game_rewards_np.shape[1]):
            ...
            for i in range(game_rewards_np.shape[0]):
                observation_strings, action_candidate_list, chosen_indices, graph_hidden_state, prev_action_strings = transition_cache[
                    i]
                is_final = True
                ...
                agent.dqn_memory.add(observation_strings[b], prev_action_strings[b], action_candidate_list[b], chosen_indices[b],
                                     graph_hidden_state[b], command_rewards_pt[i][b], graph_rewards_pt[i][b], count_rewards_pt[i][b], is_final)
                if still_running_mask_np[i][b] == 0:
                    break
            if _need_pad:
                observation_strings, action_candidate_list, chosen_indices, graph_hidden_state, prev_action_strings = transition_cache[
                    -1]
                agent.dqn_memory.add(observation_strings[b], prev_action_strings[b], action_candidate_list[b], chosen_indices[b],
                                     graph_hidden_state[b], command_rewards_pt[-1][b] * 0.0, graph_rewards_pt[-1][b] * 0.0, count_rewards_pt[-1][b] * 0.0, True)

        train_success = [1 if x else 0 for x in infos['won']]
        for b in range(batch_size):
            running_avg_game_points.push(np.sum(game_points_np, 0)[b])
            running_avg_success.push(np.mean(train_success))
        ...
        if agent.run_eval:
            eval_game_points, eval_game_points_normalized, eval_game_step, detailed_scores, eval_success = evaluate.evaluate_rl_with_real_graphs(
        ...
        _s = json.dumps({"time spent": str(time_2 - time_1).rsplit(".")[0],
                         ...
                         "eval_success_rate": str(eval_success),
                         "train_success_rate": str(np.mean(train_success)),
