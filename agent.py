...
class Agent:
    def __init__(self, config):
        ...
        self.online_net = KG_Manipulation(
            config=self.config, word_vocab=self.word_vocab, node_vocab=self.node_vocab, relation_vocab=self.relation_vocab)
        self.online_net.train()
        if self.use_cuda:
            self.online_net.cuda()

        if self.task == "rl":
            self.target_net = KG_Manipulation(
                config=self.config, word_vocab=self.word_vocab, node_vocab=self.node_vocab, relation_vocab=self.relation_vocab)
            self.pretrained_graph_generation_net = KG_Manipulation(
                config=self.config, word_vocab=self.word_vocab, node_vocab=self.node_vocab, relation_vocab=self.relation_vocab)
    ...
    def load_config(self):
        ...
        self.learning_rate_warmup_until = self.config['general'][
            'training']['optimizer']['learning_rate_warmup_until']
        ...
        self.load_graph_generation_model_from_tag = self.config['general'][
            'checkpoint']['load_graph_generation_model_from_tag']
        self.load_parameter_keywords = list(
            set(self.config['general']['checkpoint']['load_parameter_keywords']))
        ...
        self.epsilon_scheduler = LinearSchedule(
            schedule_timesteps=self.epsilon_anneal_episodes, initial_p=self.epsilon_anneal_from, final_p=self.epsilon_anneal_to)
        ...
        self.beta_scheduler = LinearSchedule(
            schedule_timesteps=self.max_episode, initial_p=self.prioritized_replay_beta, final_p=1.0)
        ...
        self.learn_start_from_this_episode = self.config[
            'rl']['training']['learn_start_from_this_episode']
    ...
    def load_pretrained_graph_generation_model(self, load_from):
        ...
        try:
            ...
            try:
                self.pretrained_graph_generation_net.load_state_dict(
                    pretrained_dict)
            except:
    ...
    def get_word_input(self, input_strings):
        ...
        word_id_list = [_words_to_ids(tokens, self.word2id)
    ...
    def get_graph_adjacency_matrix(self, triplets):
        adj = np.zeros((len(triplets), len(self.relation_vocab), len(
            self.node_vocab), len(self.node_vocab)), dtype="float32")
        for b in range(len(triplets)):
            ...
            for t in triplets[b]:
                ...
                node1_id, node2_id, relation_id = _word_to_id(node1, self.node2id), _word_to_id(
                    node2, self.node2id), _word_to_id(relation, self.relation2id)
                adj[b][relation_id][node1_id][node2_id] = 1.0
                adj[b][relation_id +
                       self.origin_relation_number][node2_id][node1_id] = 1.0
    ...
    def get_action_candidate_list_input(self, action_candidate_list):
        ...
        input_action_candidate = to_pt(
            input_action_candidate, self.use_cuda, type="long")
        for i in range(batch_size):
            input_action_candidate[i, :input_action_candidate_list[i].size(
                0), :input_action_candidate_list[i].size(1)] = input_action_candidate_list[i]
    ...
    def encode_graph(self, graph_input, use_model):
        ...
        if isinstance(graph_input, list):
            ...
            raise NotImplementedError
        node_encoding_sequence, node_mask = model.encode_graph(
            input_node_name, input_relation_name, adjacency_matrix)
    ...
    def hidden_to_adjacency_matrix(self, hidden, batch_size, use_model):
        ...
        if hidden is None:
            ...
            adjacency_matrix = torch.tanh(model.obs_gen_linear_2(F.relu(model.obs_gen_linear_1(
                hidden)))).view(batch_size, int(num_relation / 2), num_node, num_node)
            adjacency_matrix = adjacency_matrix.repeat(1, 2, 1, 1)
            for i in range(int(num_relation / 2)):
                adjacency_matrix[:, int(
                    num_relation / 2) + i] = adjacency_matrix[:, i].permute(0, 2, 1)
    ...
    def generate_adjacency_matrix_for_rl(self, observation_strings, prev_action_strings, h_t_minus_one):
        with torch.no_grad():
            ...
            prev_action_encoding_sequence, prev_action_mask = self.pretrained_graph_generation_net.encode_text_for_pretraining_tasks(
                prev_action_word_ids)
            obs_encoding_sequence, obs_mask = self.pretrained_graph_generation_net.encode_text_for_pretraining_tasks(
                input_obs)
            prev_adjacency_matrix = self.hidden_to_adjacency_matrix(h_t_minus_one, batch_size=len(
                observation_strings), use_model="pretrained_graph_generation")
            node_encoding_sequence, node_mask = self.encode_graph(
                prev_adjacency_matrix, use_model="pretrained_graph_generation")

            h_ag = self.pretrained_graph_generation_net.obs_gen_attention(
                prev_action_encoding_sequence, node_encoding_sequence, prev_action_mask, node_mask)
            h_ga = self.pretrained_graph_generation_net.obs_gen_attention(
                node_encoding_sequence, prev_action_encoding_sequence, node_mask, prev_action_mask)

            h_ag = self.pretrained_graph_generation_net.obs_gen_attention_prj(
                h_ag)
            h_ga = self.pretrained_graph_generation_net.obs_gen_attention_prj(
                h_ga)

            h_og = self.pretrained_graph_generation_net.obs_gen_attention(
                obs_encoding_sequence, node_encoding_sequence, obs_mask, node_mask)
            h_go = self.pretrained_graph_generation_net.obs_gen_attention(
                node_encoding_sequence, obs_encoding_sequence, node_mask, obs_mask)

            h_og = self.pretrained_graph_generation_net.obs_gen_attention_prj(
                h_og)  # bs X len X block_hidden_dim
            h_go = self.pretrained_graph_generation_net.obs_gen_attention_prj(
            ...
            rnn_input = self.pretrained_graph_generation_net.obs_gen_attention_to_rnn_input(
                torch.cat([ave_h_go, ave_h_og, ave_h_ga, ave_h_ag], dim=1))  # batch x block_hidden_dim
            rnn_input = torch.tanh(rnn_input)  # batch x block_hidden_dim
            h_t = self.pretrained_graph_generation_net.obs_gen_graph_rnncell(
                rnn_input, h_t_minus_one) if h_t_minus_one is not None else self.pretrained_graph_generation_net.obs_gen_graph_rnncell(rnn_input)  # both batch x block_hidden_dim
            current_adjacency_matrix = self.hidden_to_adjacency_matrix(h_t, batch_size=len(
                observation_strings), use_model="pretrained_graph_generation")
    ...
    def get_observation_infomax_logits(self, observation_strings, prev_action_strings, corrupted_observation_strings, h_t_minus_one):
        ...
        input_obs = self.get_word_input(observation_strings)
        prev_action_word_ids = self.get_word_input(prev_action_strings)
        prev_action_encoding_sequence, prev_action_mask = self.online_net.encode_text_for_pretraining_tasks(
            prev_action_word_ids)
        obs_encoding_sequence, obs_mask = self.online_net.encode_text_for_pretraining_tasks(
            input_obs)
        # TE-corrupted encode
        corrupted_input_obs = self.get_word_input(
            corrupted_observation_strings)
        corrupted_obs_encoding_sequence, corrupted_obs_mask = self.online_net.encode_text_for_pretraining_tasks(
            corrupted_input_obs)
        # adj matrix
        prev_adjacency_matrix = self.hidden_to_adjacency_matrix(
            h_t_minus_one, batch_size=len(observation_strings), use_model="online")
        node_encoding_sequence, node_mask = self.encode_graph(
            prev_adjacency_matrix, use_model="online")

        h_ag = self.online_net.obs_gen_attention(
            prev_action_encoding_sequence, node_encoding_sequence, prev_action_mask, node_mask)
        h_ga = self.online_net.obs_gen_attention(
            node_encoding_sequence, prev_action_encoding_sequence, node_mask, prev_action_mask)
        h_ag = self.online_net.obs_gen_attention_prj(h_ag)
        h_ga = self.online_net.obs_gen_attention_prj(h_ga)

        h_og = self.online_net.obs_gen_attention(
            obs_encoding_sequence, node_encoding_sequence, obs_mask, node_mask)
        h_go = self.online_net.obs_gen_attention(
            node_encoding_sequence, obs_encoding_sequence, node_mask, obs_mask)
        h_og = self.online_net.obs_gen_attention_prj(
            h_og)  # bs X len X block_hidden_dim
        h_go = self.online_net.obs_gen_attention_prj(
            h_go)  # bs X len X block_hidden_dim
        ...
        rnn_input = self.online_net.obs_gen_attention_to_rnn_input(torch.cat(
            [ave_h_go, ave_h_og, ave_h_ga, ave_h_ag], dim=1))  # batch x block_hidden_dim
        rnn_input = torch.tanh(rnn_input)  # batch x block_hidden_dim
        h_t = self.online_net.obs_gen_graph_rnncell(rnn_input, h_t_minus_one) if h_t_minus_one is not None else self.online_net.obs_gen_graph_rnncell(
            rnn_input)  # both batch x block_hidden_dim
        current_adjacency_matrix = self.hidden_to_adjacency_matrix(
            h_t, batch_size=len(observation_strings), use_model="online")
        new_node_encoding_sequence, new_node_mask = self.encode_graph(
            current_adjacency_matrix, use_model="online")

        h_ag2 = self.online_net.obs_gen_attention(
            prev_action_encoding_sequence, new_node_encoding_sequence, prev_action_mask, new_node_mask)
        h_ga2 = self.online_net.obs_gen_attention(
            new_node_encoding_sequence, prev_action_encoding_sequence, new_node_mask, prev_action_mask)
        ...
        logits = self.online_net.observation_discriminator(torch.cat(
            [ave_h_ag2, ave_h_ga2], dim=-1), obs_encoding_sequence, obs_mask, corrupted_obs_encoding_sequence, corrupted_obs_mask)
        return logits, h_t
    ...
    def get_observation_infomax_loss(self, observation_strings, prev_action_strings, evaluate=False):
        ...
        max_len = max(lens)
        episodes_masks = torch.zeros((curr_batch_size, max_len), dtype=torch.float).cuda(
        ) if self.use_cuda else torch.zeros((curr_batch_size, max_len), dtype=torch.float)
        for i in range(curr_batch_size):
            episodes_masks[i, :lens[i]] = 1
        episodes_masks = episodes_masks.repeat(
            2, 1)  # repeat for corrupted obs

        observation_strings = [
            elem + ["<pad>"]*(max_len - len(elem)) for elem in observation_strings]
        prev_action_strings = [
            elem + ["<pad>"]*(max_len - len(elem)) for elem in prev_action_strings]
        prev_h = None
        ...
        for i in range(max_len):
            ...
            for j in range(curr_batch_size):
                ...
                if lens[j] == 1 or random.random() > 0.7:
                    random_id_from_batch = random.choice(
                        range(len(observation_strings)))
                    batch_corrupted_obs_strings.append(
                        observation_strings[random_id_from_batch][random.choice(range(lens[random_id_from_batch]))])
                    while(batch_corrupted_obs_strings[-1] == batch_obs_strings[-1]):
                        random_id_from_batch = random.choice(
                            range(len(observation_strings)))
                        batch_corrupted_obs_strings[-1] = observation_strings[random_id_from_batch][random.choice(
                            range(lens[random_id_from_batch]))]
                else:
                    batch_corrupted_obs_strings.append(
                        observation_strings[j][random.choice(range(lens[j]))])
                    while(batch_corrupted_obs_strings[-1] == batch_obs_strings[-1]):
                        batch_corrupted_obs_strings[-1] = observation_strings[j][random.choice(
                            range(lens[j]))]
                batch_prev_action_strings.append(prev_action_strings[j][i])
            logits, prev_h = self.get_observation_infomax_logits(
                batch_obs_strings, batch_prev_action_strings, batch_corrupted_obs_strings, prev_h)
            ...
            loss = torch.nn.BCEWithLogitsLoss(
                reduction="none")(logits.squeeze(1), labels)
            loss = torch.sum(loss * current_step_eps_masks) / \
                torch.sum(current_step_eps_masks)
            ...
            for m in range(curr_batch_size):
                if current_step_eps_masks[m] == 0:
                    continue
                return_accuracies.append(
                    float(preds[curr_batch_size + m] == 0))
            if evaluate:
                continue
    ...
    def observation_generation_teacher_force(self, observation_strings, prev_action_strings, episodes_masks, h_t_minus_one):
        input_observation_strings = [
            " ".join(["<bos>"] + item.split()) for item in observation_strings]
        output_observation_strings = [
            " ".join(item.split() + ["<eos>"]) for item in observation_strings]
        ...
        pred = self.online_net.decode_for_obs_gen(
            input_target, h_ag2, prev_action_mask, h_ga2, new_node_mask)
        batch_loss = NegativeLogLoss(pred * target_mask.unsqueeze(-1),
                                     ground_truth, target_mask, smoothing_eps=self.smoothing_eps)
        # only place where using `episodes_masks`
        loss = torch.sum(batch_loss * episodes_masks) / \
            torch.sum(episodes_masks)
    ...
    def observation_generation_greedy_generation(self, observation_strings, prev_action_strings, episodes_masks, h_t_minus_one=None):
        with torch.no_grad():
            ...
            obs_encoding_sequence, obs_mask = model.encode_text_for_pretraining_tasks(
                input_obs)
            prev_adjacency_matrix = self.hidden_to_adjacency_matrix(
                h_t_minus_one, batch_size=len(observation_strings), use_model="online")
            node_encoding_sequence, node_mask = self.encode_graph(
                prev_adjacency_matrix, use_model="online")
            prev_action_sequence, prev_action_mask = model.encode_text_for_pretraining_tasks(
                prev_action_word_ids)

            h_ag = self.online_net.obs_gen_attention(
                prev_action_sequence, node_encoding_sequence, prev_action_mask, node_mask)
            h_ga = self.online_net.obs_gen_attention(
                node_encoding_sequence, prev_action_sequence, node_mask, prev_action_mask)
            h_ag = self.online_net.obs_gen_attention_prj(h_ag)
            h_ga = self.online_net.obs_gen_attention_prj(h_ga)
            ...
            new_node_encoding_sequence, new_node_mask = self.encode_graph(
                current_adjacency_matrix, use_model="online")
            h_ag2 = self.online_net.obs_gen_attention(
                prev_action_sequence, new_node_encoding_sequence, prev_action_mask, new_node_mask)
            h_ga2 = self.online_net.obs_gen_attention(
                new_node_encoding_sequence, prev_action_sequence, new_node_mask, prev_action_mask)
            h_ag2 = self.online_net.obs_gen_attention_prj(h_ag2)
            h_ga2 = self.online_net.obs_gen_attention_prj(h_ga2)
            ...
            for _ in range(self.max_target_length):
                ...
                input_target = self.get_word_input(
                    [" ".join(item) for item in input_target_token_list])
                pred = model.decode_for_obs_gen(
                    input_target, h_ag2, prev_action_mask, h_ga2, new_node_mask)  # batch x time x vocab
                # pointer softmax
                pred = to_np(pred[:, -1])  # batch x vocab
                pred = np.argmax(pred, -1)  # batch
                for b in range(batch_size):
                    new_stuff = [self.word_vocab[pred[b]]
                                 ] if eos[b] == 0 else []
    ...
    def command_generation_teacher_force(self, observation_strings, triplets, target_strings):
        input_target_strings = [
            " ".join(["<bos>"] + item.split()) for item in target_strings]
        output_target_strings = [
            " ".join(item.split() + ["<eos>"]) for item in target_strings]
        ...
        obs_encoding_sequence, obs_mask = self.online_net.encode_text_for_pretraining_tasks(
            input_obs)
        node_encoding_sequence, node_mask = self.encode_graph(
            triplets, use_model="online")

        h_og = self.online_net.cmd_gen_attention(
            obs_encoding_sequence, node_encoding_sequence, obs_mask, node_mask)
        h_go = self.online_net.cmd_gen_attention(
            node_encoding_sequence, obs_encoding_sequence, node_mask, obs_mask)
        ...
        target_mask = compute_mask(input_target)
        # batch x target_length x vocab
        pred = self.online_net.decode(
            input_target, h_og, obs_mask, h_go, node_mask, input_obs)
    ...
    def command_generation_greedy_generation(self, observation_strings, triplets):
        with torch.no_grad():
            ...
            obs_encoding_sequence, obs_mask = model.encode_text_for_pretraining_tasks(
                input_obs)
            node_encoding_sequence, node_mask = self.encode_graph(
                triplets, use_model="pretrained_graph_generation" if self.task == "rl" else "online")

            h_og = model.cmd_gen_attention(
                obs_encoding_sequence, node_encoding_sequence, obs_mask, node_mask)
            h_go = model.cmd_gen_attention(
                node_encoding_sequence, obs_encoding_sequence, node_mask, obs_mask)
            ...
            for _ in range(self.max_target_length):
                ...
                pred = model.decode(input_target, h_og,
    ...
    def get_action_prediction_logits(self, current_triplets, previous_triplets, target_action, action_candidates):
        ...
        h_g, node_mask = self.encode_graph(
            current_triplets, use_model="online")
        prev_h_g, prev_node_mask = self.encode_graph(
            previous_triplets, use_model="online")
        graph_mask = torch.gt(node_mask + prev_node_mask,
                              0.0).float()  # tmp_batch x num_node

        labels, action_candidate_list = generate_labels_for_ap(
            target_action, action_candidates, k_way_classification=self.ap_k_way_classification)
        input_action_candidate = self.get_action_candidate_list_input(
            action_candidate_list)  # batch x num_cand x cand_length

        batch_size = len(action_candidate_list)
        num_candidate, candidate_len = input_action_candidate.size(
            1), input_action_candidate.size(2)
        ...
        attended_h_g = self.online_net.ap_attention_prj(
            self.online_net.ap_attention(h_g, prev_h_g, node_mask, prev_node_mask))
        ave_attended_h_g = masked_mean(
            attended_h_g, graph_mask, dim=1).unsqueeze(1)
        prev_attended_h_g = self.online_net.ap_attention_prj(
            self.online_net.ap_attention(prev_h_g, h_g, prev_node_mask, node_mask))
        ave_prev_attended_h_g = masked_mean(
            prev_attended_h_g, graph_mask, dim=1).unsqueeze(1)

        # batch x num_cand x hid
        ...
        prev_global_g = prev_global_g * \
            cand_mask.unsqueeze(-1)  # batch x num_cand x hid

        cand_encoding = []
        input_action_candidate = input_action_candidate.view(
            batch_size * num_candidate, candidate_len)
        tmp_batch_size = self.batch_size if self.mode == "train" else self.eval_batch_size
        n_tmp_batches = (input_action_candidate.size(
            0) + tmp_batch_size - 1) // tmp_batch_size
        for tmp_batch_id in range(n_tmp_batches):
            tmp_batch = input_action_candidate[tmp_batch_id *
                                               tmp_batch_size: (tmp_batch_id + 1) * tmp_batch_size]
            tmp_cand_encoding_sequence, tmp_cand_mask = self.online_net.encode_text_for_pretraining_tasks(
                tmp_batch)
            tmp_cand_encoding = masked_mean(
                tmp_cand_encoding_sequence, tmp_cand_mask, dim=1)
            cand_encoding.append(tmp_cand_encoding)
        # batch_size*num_candidate x hid
        cand_encoding = torch.cat(cand_encoding, 0)
        # batch_size x num_candidate x hid
        cand_encoding = cand_encoding.view(batch_size, num_candidate, -1)
        # batch x num_candidate x hid
        cand_encoding = cand_encoding * cand_mask.unsqueeze(-1)

        # batch x num_action, hid * 3
        apmlp_input = torch.cat([global_g, prev_global_g, cand_encoding], -1)
        # batch x num_action x num_action
        cand_mask_squared = torch.bmm(
            cand_mask.unsqueeze(-1), cand_mask.unsqueeze(1))
        ap_ret, _ = self.online_net.ap_self_attention(
            apmlp_input, cand_mask_squared, apmlp_input, apmlp_input)  # batch x num_actions x hid*3
        ap_ret = ap_ret * cand_mask.unsqueeze(-1)
        ap_ret = self.online_net.ap_linear_1(
            ap_ret)  # batch x num_action x hid
        ap_ret = torch.relu(ap_ret)
        ap_ret = ap_ret * cand_mask.unsqueeze(-1)
        ap_ret = self.online_net.ap_linear_2(
            ap_ret).squeeze(-1)  # batch x num_action
    ...
    def get_state_prediction_logits(self, previous_triplets, action, target_triplets, graph_candidate_triplets):
        ...
        labels, graph_candidate_triplets_list = generate_labels_for_sp(
            target_triplets, graph_candidate_triplets, k_way_classification=self.sp_k_way_classification)
        np_labels = pad_sequences(labels)
        pt_labels = to_pt(np_labels, self.use_cuda, type='long')
        batch_size, num_candidates = np_labels.shape[0], np_labels.shape[1]

        prev_h_g, prev_node_mask = self.encode_graph(
            previous_triplets, use_model="online")  # batch x num_node x hid
        input_action = self.get_word_input(action)  # batch x action_len
        action_encoding_sequence, action_mask = self.online_net.encode_text_for_pretraining_tasks(
            input_action)  # batch x action_len x hid
        # .unsqueeze(1).expand(-1, num_candidates, -1)  # batch x num_cand x hid
        action_encoding = masked_mean(
            action_encoding_sequence, action_mask, dim=1)
        ...
        tmp_batch_size = self.batch_size if self.mode == "train" else self.eval_batch_size
        n_tmp_batches = (len(squeezed_candidate_list) +
                         tmp_batch_size - 1) // tmp_batch_size
        list_avg_graph_representation = []
        for b in range(batch_size):
            list_avg_graph_representation.append([])
        for tmp_batch_id in range(n_tmp_batches):
            tmp_batch_cand = squeezed_candidate_list[tmp_batch_id * tmp_batch_size: (
                tmp_batch_id + 1) * tmp_batch_size]  # tmp_batch of graphs
            tmp_batch_from = from_which_original_batch[tmp_batch_id * tmp_batch_size: (
                tmp_batch_id + 1) * tmp_batch_size]
            from_dict = {}
            ...
            graph_candidates_h_g, graph_candidates_mask = self.encode_graph(
                tmp_batch_cand, use_model="online")  # tmp_batch x num_node x hid, tmp_batch x num_node
            exp_prev_h_g, exp_prev_node_mask = [], []
            action_rep = []
            for which in sorted(from_dict):
                exp_prev_h_g.append(prev_h_g[which].unsqueeze(
                    0).expand(from_dict[which], -1, -1))
                exp_prev_node_mask.append(
                    prev_node_mask[which].unsqueeze(0).expand(from_dict[which], -1))
                action_rep.append(action_encoding[which].unsqueeze(
                    0).expand(from_dict[which], -1))
            exp_prev_h_g, exp_prev_node_mask = torch.cat(exp_prev_h_g, 0), torch.cat(
                exp_prev_node_mask, 0)  # tmp_batch x num_node x hid, tmp_batch x num_node
            action_rep = torch.cat(action_rep, 0).unsqueeze(
                1).expand(-1, exp_prev_h_g.size(1), -1)  # tmp_batch x num_node x hid

            attended_h_g_cand = self.online_net.sp_attention(
                exp_prev_h_g, graph_candidates_h_g, exp_prev_node_mask, graph_candidates_mask)
            attended_h_g_cand = self.online_net.sp_attention_prj(
                attended_h_g_cand)  # tmp_batch x num_node x hid

            attended_cand_h_g = self.online_net.sp_attention(
                graph_candidates_h_g, exp_prev_h_g, graph_candidates_mask, exp_prev_node_mask)
            attended_cand_h_g = self.online_net.sp_attention_prj(
                attended_cand_h_g)  # tmp_batch x num_node x hid

            # tmp_batch x num_node x hid*3
            graph_rep = torch.cat(
                [attended_h_g_cand, attended_cand_h_g, action_rep], -1)
            # tmp_batch x num_node
            graph_mask = torch.gt(exp_prev_node_mask +
                                  graph_candidates_mask, 0.0).float()
            # batch x num_node x num_node
            graph_mask_squared = torch.bmm(
                graph_mask.unsqueeze(-1), graph_mask.unsqueeze(1))
            graph_self_attended, _ = self.online_net.sp_self_attention(
                graph_rep, graph_mask_squared, graph_rep, graph_rep)
            avg_graph_self_attended = masked_mean(
                graph_self_attended, graph_mask, dim=1)  # tmp_batch x hid*3

            # unsqueeze avg_attended_h_g_cand and avg_attended_cand_h_g
            for i, which in enumerate(tmp_batch_from):
                list_avg_graph_representation[which].append(
                    avg_graph_self_attended[i])
        # paddings
        tensor_avg_graph_representation = torch.autograd.Variable(torch.zeros(
            batch_size, num_candidates, list_avg_graph_representation[0][0].size(-1)))  # batch x num_cand x hid*3
        if self.use_cuda:
            tensor_avg_graph_representation = tensor_avg_graph_representation.cuda()
        for b in range(batch_size):
            for c in range(len(list_avg_graph_representation[b])):
                tensor_avg_graph_representation[b, c,
                                                :] = list_avg_graph_representation[b][c]
        ...
        sp_ret = self.online_net.sp_linear_2(
            sp_ret).squeeze(-1)  # batch x num_cand
    ...
    def get_deep_graph_infomax_logits(self, triplets):
        ...
        node_embeddings = self.online_net.get_graph_node_representations(
            input_node_name)  # num_node x hid
        relation_embeddings = self.online_net.get_graph_relation_representations(
            input_relation_name)  # num_relation x hid
        node_embeddings = node_embeddings.repeat(
            batch_size, 1, 1)  # batch x num_node x hid
        relation_embeddings = relation_embeddings.repeat(
            batch_size, 1, 1)  # batch x num_relation x hid

        label_positive = torch.ones(batch_size, node_embeddings.size(1))
        label_negative = torch.zeros(batch_size, node_embeddings.size(1))
        labels = torch.cat((label_positive, label_negative),
                           1)  # batch x num_node*2
        if self.use_cuda:
            labels = labels.cuda()

        # get node mask
        # batch x num_nodes x num_nodes
        node_mask = torch.sum(adjacency_matrices[:, :-1, :, :], 1)
        ...
        h_positive, h_negative, global_representations = self.online_net.get_deep_graph_infomax_discriminator_input(
            node_embeddings, shuffled_node_embeddings, node_mask, relation_embeddings, adjacency_matrices)
        dgi_discriminator_logits = self.online_net.dgi_discriminator(
            global_representations, h_positive, h_negative, self.sample_bias_positive, self.sample_bias_negative)  # batch x num_node*2

        loss = torch.nn.BCEWithLogitsLoss(reduction="none")(
            dgi_discriminator_logits, labels)
        loss = torch.sum(
            loss * torch.cat((node_mask, node_mask), 1), 1) / (2 * node_mask.sum(1))
    ...
    def get_game_info_at_certain_step_fully_observable(self, obs, infos):
        ...
        batch_size = len(obs)
        observation_strings = [
            preproc(item, tokenizer=self.nlp) for item in obs]
        action_candidate_list = []
        for b in range(batch_size):
            ac = [preproc(item, tokenizer=self.nlp)
                  for item in infos["admissible_commands"][b]]
            action_candidate_list.append(ac)

        # get new facts
        current_triplets = []  # batch of list of triplets
        for b in range(batch_size):
            new_f = set(process_fully_obs_facts(
                infos["game"][b], infos["facts"][b]))
    ...
    def get_game_info_at_certain_step(self, obs, infos, prev_actions=None, prev_facts=None, return_gt_commands=False):
        ...
        for b in range(batch_size):
            if prev_facts is None:
                new_f = process_facts(
                    None, infos["game"][b], infos["facts"][b], None, None)
                prev_f = set()
            else:
                new_f = process_facts(
                    prev_facts[b], infos["game"][b], infos["facts"][b], infos["last_action"][b], prev_actions[b])
                prev_f = prev_facts[b]
            new_facts.append(new_f)
            triplets = serialize_facts(new_f)
            current_triplets.append(triplets)
            target_commands = gen_graph_commands(
                new_f - prev_f, cmd="add") + gen_graph_commands(prev_f - new_f, cmd="delete")
        ...
        if return_gt_commands:
            ...
            target_command_strings = [" <sep> ".join(
                sort_target_commands(tgt_cmds)) for tgt_cmds in commands_from_env]
    ...
    def update_knowledge_graph_triplets(self, triplets, prediction_strings):
        ...
        for i in range(len(triplets)):
            ...
            new_triplets.append(update_graph_triplets(
                triplets[i], predict_cmds, self.node_vocab, self.relation_vocab))
        return new_triplets
    ...
    def encode(self, observation_strings, graph_input, use_model):
        ...
        if self.enable_text_input:
            obs_encoding_sequence, obs_mask = self.encode_text(
                observation_strings, use_model=use_model)
        else:
            obs_encoding_sequence, obs_mask = None, None

        if self.enable_graph_input:
            node_encoding_sequence, node_mask = self.encode_graph(
                graph_input, use_model=use_model)
        else:
            node_encoding_sequence, node_mask = None, None

        if self.enable_text_input and self.enable_graph_input:
            h_og, h_go = model.get_match_representations(
                obs_encoding_sequence, obs_mask, node_encoding_sequence, node_mask)
    ...
    def action_scoring(self, action_candidate_list, h_og=None, obs_mask=None, h_go=None, node_mask=None, previous_h=None, previous_c=None, use_model=None):
        ...
        action_scores, action_masks, new_h, new_c = model.score_actions(
            input_action_candidate, h_og, obs_mask, h_go, node_mask, previous_h, previous_c)  # batch x num_actions
    ...
    def act_greedy(self, observation_strings, graph_input, action_candidate_list, previous_h=None, previous_c=None):
        with torch.no_grad():
            h_og, obs_mask, h_go, node_mask = self.encode(
                observation_strings, graph_input, use_model="online")
            action_scores, action_masks, new_h, new_c = self.action_scoring(
                action_candidate_list, h_og, obs_mask, h_go, node_mask, previous_h, previous_c, use_model="online")
            ...
            chosen_actions = [item[idx] for item, idx in zip(
    ...
    def act_random(self, observation_strings, graph_input, action_candidate_list, previous_h=None, previous_c=None):
        with torch.no_grad():
            h_og, obs_mask, h_go, node_mask = self.encode(
                observation_strings, graph_input, use_model="online")
            action_scores, _, new_h, new_c = self.action_scoring(
                action_candidate_list, h_og, obs_mask, h_go, node_mask, previous_h, previous_c, use_model="online")
    ...
    def act(self, observation_strings, graph_input, action_candidate_list, previous_h=None, previous_c=None, random=False):
        ...
        with torch.no_grad():
            ...
            chosen_indices = less_than_epsilon * action_indices_random + \
                greater_than_epsilon * action_indices_maxq
            chosen_indices = chosen_indices.astype(int)
            chosen_actions = [item[idx] for item, idx in zip(
                action_candidate_list, chosen_indices)]
    ...
    def get_dqn_loss_with_real_graphs(self, episode_no):
        ...
        if len(self.dqn_memory) < self.replay_batch_size:
            return None, None
        data = self.dqn_memory.sample(self.replay_batch_size, beta=self.beta_scheduler.value(
            episode_no), multi_step=self.multi_step)
        if data is None:
            return None, None
        ...
        prev_graph_hidden_state = to_pt(
            np.stack(prev_graph_hidden_state, 0), enable_cuda=self.use_cuda, type='float')
        new_adjacency_matrix, _ = self.generate_adjacency_matrix_for_rl(
            obs_list, prev_action_list, prev_graph_hidden_state)
        next_prev_graph_hidden_state = to_pt(np.stack(
            next_prev_graph_hidden_state, 0), enable_cuda=self.use_cuda, type='float')
        next_new_adjacency_matrix, _ = self.generate_adjacency_matrix_for_rl(
            next_obs_list, next_prev_action_list, next_prev_graph_hidden_state)

        h_og, obs_mask, h_go, node_mask = self.encode(
            obs_list, new_adjacency_matrix, use_model="online")
        action_scores, _, _, _ = self.action_scoring(
            candidate_list, h_og, obs_mask, h_go, node_mask, None, None, use_model="online")

        # ps_a
        action_indices = to_pt(
            action_indices, enable_cuda=self.use_cuda, type='long').unsqueeze(-1)
        q_value = ez_gather_dim_1(
            action_scores, action_indices).squeeze(1)  # batch
        ...
        with torch.no_grad():
            ...
            h_og, obs_mask, h_go, node_mask = self.encode(
                next_obs_list, next_new_adjacency_matrix, use_model="online")
            next_action_scores, next_action_masks, _, _ = self.action_scoring(
                next_candidate_list, h_og, obs_mask, h_go, node_mask, None, None, use_model="online")

            # Perform argmax action selection using online network: argmax_a[(z, p(s_t+n, a; θonline))]
            next_action_indices = self.choose_maxQ_action(
                next_action_scores, next_action_masks)  # batch
            next_action_indices = to_pt(
                next_action_indices, enable_cuda=self.use_cuda, type='long').unsqueeze(-1)
            # pns # Probabilities p(s_t+n, ·; θtarget)
            h_og, obs_mask, h_go, node_mask = self.encode(
                next_obs_list, next_new_adjacency_matrix, use_model="target")
            next_action_scores, next_action_masks, _, _ = self.action_scoring(
                next_candidate_list, h_og, obs_mask, h_go, node_mask, None, None, use_model="target")
            ...
            discount = to_pt((np.ones_like(
                actual_ns) * self.discount_gamma_game_reward) ** actual_ns, self.use_cuda, type="float")

        rewards = rewards + next_q_value * discount  # batch
        loss = F.smooth_l1_loss(q_value, rewards, reduce=False)  # batch

        prior_weights = to_pt(
            prior_weights, enable_cuda=self.use_cuda, type="float")
        loss = loss * prior_weights
        loss = torch.mean(loss)

        abs_td_error = np.abs(to_np(q_value - rewards))
        new_priorities = abs_td_error + self.prioritized_replay_eps
        success = self.dqn_memory.update_priorities(
            actual_indices, new_priorities)
        if not success:
            return None, None
    ...
    def get_dqn_loss(self, episode_no):
        ...
        h_og, obs_mask, h_go, node_mask = self.encode(
            obs_list, graph_triplet_list, use_model="online")
        action_scores, _, _, _ = self.action_scoring(
            candidate_list, h_og, obs_mask, h_go, node_mask, None, None, use_model="online")
        ...
        with torch.no_grad():
            ...
            # pns Probabilities p(s_t+n, ·; θonline)
            h_og, obs_mask, h_go, node_mask = self.encode(
                next_obs_list, next_graph_triplet_list, use_model="online")
            next_action_scores, next_action_masks, _, _ = self.action_scoring(
                next_candidate_list, h_og, obs_mask, h_go, node_mask, None, None, use_model="online")
            ...
            # pns # Probabilities p(s_t+n, ·; θtarget)
            h_og, obs_mask, h_go, node_mask = self.encode(
                next_obs_list, next_graph_triplet_list, use_model="target")
            next_action_scores, next_action_masks, _, _ = self.action_scoring(
                next_candidate_list, h_og, obs_mask, h_go, node_mask, None, None, use_model="target")
    ...
    def get_drqn_loss(self, episode_no):
        ...
        data = self.dqn_memory.sample_sequence(self.replay_batch_size, beta=self.beta_scheduler.value(
            episode_no), sample_history_length=self.replay_sample_history_length)
        ...
        for step_no in range(self.replay_sample_history_length):
            ...
            h_og, obs_mask, h_go, node_mask = self.encode(
                obs_list, graph_triplet_list, use_model="online")
            action_scores, _, new_h, new_c = self.action_scoring(
                candidate_list, h_og, obs_mask, h_go, node_mask, prev_h, prev_c, use_model="online")
            ...
            with torch.no_grad():
                ...
                h_og, obs_mask, h_go, node_mask = self.encode(
                    next_obs_list, next_graph_triplet_list, use_model="online")
                next_action_scores, next_action_masks, _, _ = self.action_scoring(
                    next_candidate_list, h_og, obs_mask, h_go, node_mask, prev_h, prev_c, use_model="online")
                ...
                h_og, obs_mask, h_go, node_mask = self.encode(
                    next_obs_list, next_graph_triplet_list, use_model="target")
                next_action_scores, next_action_masks, _, _ = self.action_scoring(
                    next_candidate_list, h_og, obs_mask, h_go, node_mask, prev_h, prev_c, use_model="target")
            ...
            p_weights = to_pt(
                prior_weights[step_no], enable_cuda=self.use_cuda, type="float")
            loss = loss * p_weights
        ...
        for i in range(self.replay_sample_history_length - self.replay_sample_update_from):
            ...
            self.dqn_memory.update_priorities(
                [item + self.replay_sample_update_from + i for item in actual_indices], new_priorities)
    ...
    def get_graph_rewards(self, prev_triplets, current_triplets):
        ...
        if self.graph_reward_type == "triplets_increased":
            rewards = [float(len(c_triplet) - len(p_triplet))
                       for p_triplet, c_triplet in zip(prev_triplets, current_triplets)]
            ...
            raise NotImplementedError
        rewards = [min(1.0, max(0.0, float(item) * self.graph_reward_lambda))
                   for item in rewards]
        return rewards
