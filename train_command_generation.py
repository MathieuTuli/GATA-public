...
def train():
    ...
    if agent.load_pretrained:
        if os.path.exists(output_dir + "/" + agent.experiment_tag + "_model.pt"):
            ...
            agent.load_pretrained_model(
                data_dir + "/" + agent.load_graph_update_model_from_tag + ".pt", load_partial_graph=False)
    ...
    try:
        while(True):
            ...
            _, loss = agent.command_generation_teacher_force(
                observation_strings, triplets, target_strings)
            ...
            if batch_no < agent.learning_rate_warmup_until:
                cr = agent.init_learning_rate / \
                    math.log2(agent.learning_rate_warmup_until)
            ...
            if episode_no % agent.report_frequency <= (episode_no - curr_batch_size) % agent.report_frequency:
                if agent.run_eval:
                    eval_f1_exact, eval_f1_soft = evaluate.evaluate_pretrained_command_generation(
                        env, agent, "valid")
                    env.split_reset("train")
            ...
            print("Episode: {:3d} | time spent: {:s} | loss: {:2.3f} | valid exact f1: {:2.3f} | valid soft f1: {:2.3f}".format(
            ...
            if config["general"]["visdom"]:
                ...
                if plt_win is None:
                    plt_win = viz.line(X=viz_x, Y=viz_loss,
                                       opts=dict(
                                           title=agent.experiment_tag + "_loss"),
                                       name="training loss")
                else:
                ...
                if eval_plt_win is None:
                    eval_plt_win = viz.line(X=viz_eval_x, Y=viz_eval_exact_f1,
                                            opts=dict(
                                                title=agent.experiment_tag + "_exact_f1"),
                                            name="eval exact f1")
                    viz.line(X=viz_eval_x, Y=viz_eval_soft_f1,
                             opts=dict(
                                 title=agent.experiment_tag + "_soft_f1"),
    ...
    if agent.run_eval:
        if os.path.exists(output_dir + "/" + agent.experiment_tag + "_model.pt"):
            ...
            agent.load_pretrained_model(
                output_dir + "/" + agent.experiment_tag + "_model.pt", load_partial_graph=False)
        _, _ = evaluate.evaluate_pretrained_command_generation(
            env, agent, "test", verbose=True)
